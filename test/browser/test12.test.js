...
describe('Demo ZipEntry.prototype.getFileEntry (Directory)', () => {

  let resolve;
  let reject;

  it('should handle ZipEntry.prototype.getFileEntry (Directory)', async () => {
    setZipConfig();
  ...
  const URL = 'base/test/browser/lorem2.zip';
  ...
  function importZipToFilesystem(callback) {
    zipFs.importHttpContent(URL, false, function() {
      zipFs.root.getFileEntry(filesystem.root, callback, null, reject);
    }, reject);
  }
